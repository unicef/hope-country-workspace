...
from country_workspace.stream.callbacks import OCR_RESULT_ROUTING_KEY, handle_event
...
def test_handle_event_dispatches_ocr_result(mocker):
    ...
    result = handle_event(OCR_RESULTS_QUEUE, mocker.Mock(), method, mocker.Mock(), _make_event_body(payload))
...
def test_handle_event_ignores_unknown_routing_key(mocker):
    ...
    result = handle_event(OCR_RESULTS_QUEUE, mocker.Mock(), method, mocker.Mock(), _make_event_body({"a": 1}))
...
def test_handle_event_swallows_handler_exception_and_still_acks(mocker):
    ...
    result = handle_event(
        OCR_RESULTS_QUEUE, mocker.Mock(), method, mocker.Mock(), _make_event_body({"correlation_id": "x"})
    )
...
def test_handle_event_logs_unexpected_payload_shape_without_raising(mocker):
    ...
    result = handle_event(
        OCR_RESULTS_QUEUE, mocker.Mock(), method, mocker.Mock(), _make_event_body({"unexpected": True})
    )
...
def test_handle_event_real_handler_ignores_unknown_correlation_id():
    ...
    class Method:
        routing_key = OCR_RESULT_ROUTING_KEY

    result = handle_event(OCR_RESULTS_QUEUE, None, Method(), None, body)
