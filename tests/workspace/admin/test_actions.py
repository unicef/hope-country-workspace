...
def test_post_selected_actions(mocker: MockerFixture, value, expected) -> None:
    request = mocker.MagicMock()
    request.POST = {} if value is None else {"_selected_action": value}

    assert _post_selected_actions(request) == expected
...
def test_bulk_update_export_includes_error_columns_when_requested(
    ...
    form.cleaned_data = {"fields": ["given_name"], "include_errors": True}
    mocker.patch("country_workspace.workspaces.admin.cleaners.actions.BulkUpdateExportForm", return_value=form)
    ...
    assert response == redirect.return_value
    assert create.call_args.kwargs["config"]["columns"] == ["id", "version", "given_name", "is_valid", "errors"]
